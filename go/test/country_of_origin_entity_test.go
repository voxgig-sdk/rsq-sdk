...
func TestCountryOfOriginEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.CountryOfOrigin(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.CountryOfOrigin(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		countryOfOriginRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.country_of_origin")))
