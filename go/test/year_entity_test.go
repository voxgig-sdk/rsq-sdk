...
func TestYearEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Year(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Year(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set RSQ_TEST_YEAR_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		yearRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.year")))
		...
		yearRef01Match := map[string]any{}

		yearRef01ListResult, err := yearRef01Ent.List(yearRef01Match, nil)
		if err != nil {
			t.Fatalf("list failed: %v", err)
		}
		_, yearRef01ListOk := yearRef01ListResult.([]any)
		if !yearRef01ListOk {
			t.Fatalf("expected list result to be an array, got %T", yearRef01ListResult)
		}
...
func yearBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("RSQ_TEST_YEAR_ENTID")
	...
	idmapResolved := core.ToMapAny(env["RSQ_TEST_YEAR_ENTID"])
