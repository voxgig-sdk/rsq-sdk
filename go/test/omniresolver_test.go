...
import (
	...
	omni "github.com/voxgig-sdk/rsq-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.RsqError); is {
				out := map[string]any{
					"name":    "RsqError",
...
func omniCtx(arg any, client *sdk.RsqSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.RsqSDK); is {
			client = live
