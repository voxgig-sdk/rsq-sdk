...
func TestDepartureEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set RSQ_TEST_DEPARTURE_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		departureRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.departure")))
...
func departureBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap, _ := vs.Transform(
		[]any{"departure01", "departure02", "departure03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("RSQ_TEST_DEPARTURE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["RSQ_TEST_DEPARTURE_ENTID"])
