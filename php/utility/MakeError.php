...
class RsqMakeError
{
    public static function call(?RsqContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new RsqContext([], null);
        }
        $op = $ctx->op ?? new RsqOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new RsqResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof RsqError) ? $err->msg : (string)$err;
