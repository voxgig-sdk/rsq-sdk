...
//    only - a typed RsqContext there would make every ctx
...
final class RsqOmni
{
    ...
    public static function err_from_map($m): ?RsqError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new RsqError(is_string($code) ? $code : '', $msg);
    }
