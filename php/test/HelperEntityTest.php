...
class HelperEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = RsqSDK::test(null, null);
        $ent = $testsdk->Helper(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = helper_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "helper." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set RSQ_TEST_HELPER_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $helper_ref01_data = null;
        if (count($helper_ref01_data_raw) > 0) {
...
function helper_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/helper/HelperTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["helper01", "helper02", "helper03"] as $k) {
    ...
    $entid_env_raw = getenv("RSQ_TEST_HELPER_ENTID");
