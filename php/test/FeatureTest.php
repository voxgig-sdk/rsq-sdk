...
class FtCtrl extends RsqControl
{
...
class FtClient
{
    ...
    private RsqUtility $utility;

    public function __construct(RsqUtility $utility, array $options)
    {
    ...
    public function get_utility(): RsqUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (RsqContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (RsqContext $_ctx, string $_url, array $fetchdef): array {
    ...
    private static function buildUrl(RsqSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof RsqError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new RsqResponse($fetched);
        }
    ...
    private function fail(RsqContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (RsqContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
