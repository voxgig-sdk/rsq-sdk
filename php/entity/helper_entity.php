...
class HelperEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new HelperEntity($this->_client, $opts);
    }

    /**
     * @param Helper|array $args Helper data (assoc-array) to store.
     */
    ...
    /**
     * @return Helper|array The current Helper data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Helper fields).
     */
    ...
    /**
     * @return array The current match filter (any subset of Helper fields).
     */
     ...
     *
     * @param HelperLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed HelperLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Helper|array The loaded Helper as an assoc-array at the
     *   SDK boundary; throws RsqError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = RsqHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
