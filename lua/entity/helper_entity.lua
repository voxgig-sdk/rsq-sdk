...
local HelperEntity = {}
HelperEntity.__index = HelperEntity


function HelperEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, HelperEntity)
  self._name = "helper"
  self._client = client
...
function HelperEntity:get_name()
  return self._name
end


function HelperEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return HelperEntity.new(self._client, opts)
end
...
function HelperEntity:mark_deleted()
  self._deleted = true
end


function HelperEntity:deleted()
  return true == self._deleted
end


function HelperEntity:data_set(args)
  if args ~= nil then
...
function HelperEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function HelperEntity:match_set(args)
  if args ~= nil then
...
function HelperEntity:match_get()
...
function HelperEntity:stream(action, args, callopts)
...
---@param reqmatch HelperLoadMatch
---@param ctrl? table
---@return Helper
---@return string? err
function HelperEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function HelperEntity:_run_op(ctx, post_done)
  local utility = self._utility
