...
local UrlFetchEntity = {}
UrlFetchEntity.__index = UrlFetchEntity


function UrlFetchEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, UrlFetchEntity)
  self._name = "url_fetch"
  self._client = client
...
function UrlFetchEntity:get_name()
  return self._name
end


function UrlFetchEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return UrlFetchEntity.new(self._client, opts)
end
...
function UrlFetchEntity:mark_deleted()
  self._deleted = true
end


function UrlFetchEntity:deleted()
  return true == self._deleted
end


function UrlFetchEntity:data_set(args)
  if args ~= nil then
...
function UrlFetchEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function UrlFetchEntity:match_set(args)
  if args ~= nil then
...
function UrlFetchEntity:match_get()
...
function UrlFetchEntity:stream(action, args, callopts)
...
---@param reqmatch UrlFetchListMatch
---@param ctrl? table
---@return UrlFetch[]
---@return string? err
function UrlFetchEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function UrlFetchEntity:_run_op(ctx, post_done)
  local utility = self._utility
