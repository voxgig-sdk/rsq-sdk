-- Rsq SDK Submission entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local SubmissionEntity = {}
SubmissionEntity.__index = SubmissionEntity


function SubmissionEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SubmissionEntity)
  self._name = "submission"
  self._client = client
...
function SubmissionEntity:get_name()
  return self._name
end


function SubmissionEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SubmissionEntity.new(self._client, opts)
end
...
function SubmissionEntity:mark_deleted()
  self._deleted = true
end


function SubmissionEntity:deleted()
  return true == self._deleted
end


function SubmissionEntity:data_set(args)
  if args ~= nil then
...
function SubmissionEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SubmissionEntity:match_set(args)
  if args ~= nil then
...
function SubmissionEntity:match_get()
...
function SubmissionEntity:stream(action, args, callopts)
...
function SubmissionEntity:_run_op(ctx, post_done)
  local utility = self._utility
