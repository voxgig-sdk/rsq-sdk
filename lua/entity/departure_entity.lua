...
local DepartureEntity = {}
DepartureEntity.__index = DepartureEntity


function DepartureEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DepartureEntity)
  self._name = "departure"
  self._client = client
...
function DepartureEntity:get_name()
  return self._name
end


function DepartureEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DepartureEntity.new(self._client, opts)
end
...
function DepartureEntity:mark_deleted()
  self._deleted = true
end


function DepartureEntity:deleted()
  return true == self._deleted
end


function DepartureEntity:data_set(args)
  if args ~= nil then
...
function DepartureEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DepartureEntity:match_set(args)
  if args ~= nil then
...
function DepartureEntity:match_get()
...
function DepartureEntity:stream(action, args, callopts)
...
function DepartureEntity:_run_op(ctx, post_done)
  local utility = self._utility
