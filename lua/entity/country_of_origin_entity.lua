-- Rsq SDK CountryOfOrigin entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local CountryOfOriginEntity = {}
CountryOfOriginEntity.__index = CountryOfOriginEntity


function CountryOfOriginEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CountryOfOriginEntity)
  self._name = "country_of_origin"
  self._client = client
...
function CountryOfOriginEntity:get_name()
  return self._name
end


function CountryOfOriginEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CountryOfOriginEntity.new(self._client, opts)
end
...
function CountryOfOriginEntity:mark_deleted()
  self._deleted = true
end


function CountryOfOriginEntity:deleted()
  return true == self._deleted
end


function CountryOfOriginEntity:data_set(args)
  if args ~= nil then
...
function CountryOfOriginEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CountryOfOriginEntity:match_set(args)
  if args ~= nil then
...
function CountryOfOriginEntity:match_get()
...
function CountryOfOriginEntity:stream(action, args, callopts)
...
---@param reqmatch CountryOfOriginListMatch
---@param ctrl? table
---@return CountryOfOrigin[]
---@return string? err
function CountryOfOriginEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function CountryOfOriginEntity:_run_op(ctx, post_done)
  local utility = self._utility
