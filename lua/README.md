...
The Lua SDK for the Rsq API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Category(nil):list(nil, nil)
...
local result, err = client:Rsq(nil):load(
  { id = "test01" }, nil
)
