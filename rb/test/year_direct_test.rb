...
class YearDirectTest < Minitest::Test
  def test_direct_list_year
    setup = year_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-year", setup[:live] ? "live" : "unit")
    if _should_skip
