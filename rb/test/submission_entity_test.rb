...
class SubmissionEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set RSQ_TEST_SUBMISSION_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # Bootstrap entity data from existing test data.
    submission_ref01_data_raw = Vs.items(Helpers.to_map(
      Vs.getpath(setup[:data], "existing.submission")))
    submission_ref01_data = nil
    if submission_ref01_data_raw.length > 0
      submission_ref01_data = Helpers.to_map(submission_ref01_data_raw[0][1])
    end
    ...
    submission_ref01_list_result, err = submission_ref01_ent.list(submission_ref01_match, nil)
    assert_nil err
...
def submission_basic_setup(extra)
  ...
  entid_env_raw = ENV["RSQ_TEST_SUBMISSION_ENTID"]
