...
class DepartureEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Departure(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Departure(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = departure_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "departure." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set RSQ_TEST_DEPARTURE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    departure_ref01_data = nil
    if departure_ref01_data_raw.length > 0
...
def departure_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "departure", "DepartureTestData.json")
  ...
  entid_env_raw = ENV["RSQ_TEST_DEPARTURE_ENTID"]
