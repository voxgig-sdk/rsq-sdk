...
class CountryOfAsylumEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.CountryOfAsylum(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.CountryOfAsylum(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = country_of_asylum_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "country_of_asylum." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set RSQ_TEST_COUNTRY_OF_ASYLUM_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    country_of_asylum_ref01_data = nil
    if country_of_asylum_ref01_data_raw.length > 0
...
def country_of_asylum_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "country_of_asylum", "CountryOfAsylumTestData.json")
  ...
  entid_env_raw = ENV["RSQ_TEST_COUNTRY_OF_ASYLUM_ENTID"]
