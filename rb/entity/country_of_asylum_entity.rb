...
class CountryOfAsylumEntity
  ...
  def make
    opts = @_entopts.dup
    CountryOfAsylumEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = RsqHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [CountryOfAsylum, Hash] the current CountryOfAsylum data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = RsqHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of CountryOfAsylum fields)
  def match_get
  ...
  #
  # @param reqmatch [CountryOfAsylumListMatch, Hash, nil] match filter (any subset of
  #   CountryOfAsylum fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<CountryOfAsylum>, Array] the matching CountryOfAsylum items; raises RsqError on failure
