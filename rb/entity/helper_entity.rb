...
class HelperEntity
  ...
  # @return [Helper, Hash] the current Helper data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of Helper fields)
  def match_get
  ...
  #
  # @param reqmatch [HelperLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Helper.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Helper, Hash] the loaded Helper; raises RsqError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = RsqHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
