...
class SubmissionEntity
  ...
  def make
    opts = @_entopts.dup
    SubmissionEntity.new(@_client, opts)
  end
  ...
  # @return [Submission, Hash] the current Submission data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of Submission fields)
  def match_get
  ...
  #
  # @param reqmatch [SubmissionListMatch, Hash, nil] match filter (any subset of
  #   Submission fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Submission>, Array] the matching Submission items; raises RsqError on failure
