...
class DemographicEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return DemographicEntity(self._client, opts)
    ...
    def match_get(self) -> Demographic:
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Demographic]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Demographic().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
