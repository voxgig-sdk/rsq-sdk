    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set RSQ_TEST_SUBMISSION_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        submission_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.submission")))
        submission_ref01_data = None
        if len(submission_ref01_data_raw) > 0:
        ...
        submission_ref01_list_result, err = submission_ref01_ent.list(submission_ref01_match, None)
        assert err is None
...
def _submission_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("RSQ_TEST_SUBMISSION_ENTID"))
