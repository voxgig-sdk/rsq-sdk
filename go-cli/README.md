...
boru-driven command-line client **and** interactive REPL for the Rsq
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/rsq-cli)
make build
...
./rsq-cli list country_of_asylum

# 5. Override the API base URL for a single call
RSQ_BASE=https://api.example.com ./rsq-cli list category
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/rsq-cli
   ```
...
Run with no arguments to open a REPL (prompt `rsq>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/rsq-cli`. |
...
(`./rsq-cli <expr>`) and interactively (the REPL), and expressions compose the
