...
[MCP](https://modelcontextprotocol.io) server exposing the Rsq SDK as
two agent tools — `rsq_list` and `rsq_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/rsq-mcp)
make build

# 2. Provide credentials via the environment
export RSQ_APIKEY=sk_live_xxx
...
```jsonc
// rsq_list: first page of records
{ "entity": "category" }
{ "entity": "category", "query": { } }

// rsq_load: one record by id
{ "entity": "helper", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/rsq-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user rsq \
     -- "$PWD"/dist/*/rsq-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `rsq_list` and `rsq_load` tools now appear
   in new sessions. Ask the agent to *"list category using rsq"*
   and it calls `rsq_list` with `{"entity":"category"}`.
...
```sh
export RSQ_APIKEY=sk_live_xxx            # API key
export RSQ_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `rsq_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `rsq_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `RSQ_APIKEY` | API key sent with every request. |
| `RSQ_BASE` | Optional override of the API base URL. |
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"rsq_load","arguments":{"entity":"helper","query":{"id":1}}}}'
```
