...
type Args struct {
	Entity string         `json:"entity" jsonschema:"category | country_of_asylum | country_of_origin | country_of_resettlement | demographic | departure | helper | region | submission | url_fetch | year"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.RsqSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "rsq_list",
		Description: "List records from Rsq. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "rsq_load",
		Description: "Load a single record from Rsq. " +
...
func runOp(client *sdk.RsqSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.RsqSDK, name string) (sdk.RsqEntity, error) {
	switch strings.ToLower(name) {
	case "category":
		return client.Category(nil), nil
	case "country_of_asylum":
		return client.CountryOfAsylum(nil), nil
	case "country_of_origin":
		return client.CountryOfOrigin(nil), nil
	case "country_of_resettlement":
		return client.CountryOfResettlement(nil), nil
	case "demographic":
		return client.Demographic(nil), nil
	case "departure":
		return client.Departure(nil), nil
	case "helper":
		return client.Helper(nil), nil
	case "region":
		return client.Region(nil), nil
	case "submission":
		return client.Submission(nil), nil
	case "url_fetch":
		return client.UrlFetch(nil), nil
	case "year":
