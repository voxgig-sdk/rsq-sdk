...
describe('CountryOfAsylumDirect', async () => {
  ...
  test('direct-list-country_of_asylum', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-country_of_asylum', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['RSQ_TEST_COUNTRY_OF_ASYLUM_ENTID']
