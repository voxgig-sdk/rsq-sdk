...
//     (\`new RsqSDK(...)\` / \`RsqSDK.test(...)\` become
//     \`RsqSDK.test({ entity: ... })\`), transpiled to JS, and run
...
// rewrite real client construction (\`new RsqSDK(...)\` and any
// \`RsqSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+RsqSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/RsqSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
