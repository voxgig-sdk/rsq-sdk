...
(0, node_test_1.describe)('DepartureEntity', async () => {
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        for (const op of ['list']) {
            if ((0, utility_1.maybeSkipControl)(t, 'entityOp', 'departure.' + op, live))
                return;
        }
...
function basicSetup(extra) {
    ...
    let idmap = transform(['departure01', 'departure02', 'departure03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    const idmapEnvVal = process.env['RSQ_TEST_DEPARTURE_ENTID'];
